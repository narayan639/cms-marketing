...
const MyBusiness_self: React.FC<Myself> = ({ currUser_log, isLoading }) => {
  // Sorting the currUser_log array by date in descending order
  const sortedUserLog = currUser_log?.slice().sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()) || [];

  const all_amount = sortedUserLog.map((item: any) => Number(item?.budget)) || [];
  const sum = all_amount.reduce((acc, num) => acc + num, 0);
  const total_insen_amt = (sum * 10) / 100;

  return (
    <>
      {sortedUserLog.length > 0 ? (
        <>
          {!isLoading ? (
            <Table className="rounded-[10px]">
              ...
              {sortedUserLog.map((item: any, index: number) => {
