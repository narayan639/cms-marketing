...
export const province_list=Country_data?.provinceList.map((item)=>item.name)

export const district_list=({pro}:{pro:string})=>{
    const data = Country_data?.provinceList?.find(item => item.name === pro)?.districtList;
    if (data) {
        const districtNames = data.map(item => item.name);
        return districtNames;
    } else {
        return [];
    }
}
export const municipility_list=({province,district}:{province: string, district: string})=>{
    const data = Country_data?.provinceList?.find(item => item.name === province)?.districtList;
    const data2 = data?.find((item)=>item.name===district)?.municipalityList
    if (data && data2) {
        const municipality = data2.map(item => item.name);
        return municipality
    } else {
        return [];
    }
